...
if not CODART_TOKEN:
    raise RuntimeError("Falta la variable de entorno CODART_TOKEN")
...
# ================== KEEP ALIVE SIN FLASK ==================
def iniciar_keep_alive():
    """
    Hilo keep-alive sin Flask.
    - Si KEEP_ALIVE_URL o RENDER_EXTERNAL_URL están configurados, hace ping externo para evitar sleep en Render.
    - Si no hay URL, solo mantiene un heartbeat en logs para que veas que el proceso sigue vivo.
    """
    url = (KEEP_ALIVE_URL or RENDER_EXTERNAL_URL or "").strip()
    if url:
        url = url.rstrip("/") + "/keep-alive" if not url.endswith("/keep-alive") and "telegram" not in url else url
        logger.info(f"💓 Keep-alive externo activo: {url} cada {KEEP_ALIVE_INTERVAL}s")
        def loop():
            while True:
                try:
                    ...
                    logger.warning(f"⚠️ Keep-alive falló: {exc}")
                time.sleep(KEEP_ALIVE_INTERVAL)
    else:
        logger.info(f"💓 Keep-alive interno activo cada {KEEP_ALIVE_INTERVAL}s (sin ping externo)")
        def loop():
            while True:
                logger.info("💓 Bot vivo — heartbeat")
                time.sleep(KEEP_ALIVE_INTERVAL)

    t = Thread(target=loop, daemon=True, name="keep-alive")
    t.start()
    return t
...
async def staff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    mensaje_usuario = " ".join(context.args) if context.args else "Sin mensaje"
    texto = f"""
👑 PANEL DE STAFF
━━━━━━━━━━━━━━━━━━━━━━━━
...
async def facial_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not rostros:
            agregar_creditos(update.effective_user.id, COSTOS["facial"])
            await update.message.reply_text("❌ No se detectó ningún rostro. Créditos devueltos.")
            return
        mensaje = f"""
╔══════════════╗
  🧬 FACIAL
