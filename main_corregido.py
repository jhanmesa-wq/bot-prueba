...
from telegram import Update, InputMediaPhoto, MessageEntity, InlineKeyboardButton, InlineKeyboardMarkup, Message, Bot
...
MAX_PRICE = _get_env("GRIZZLY_MAX_PRICE","MAX_PRICE","maxPrice")

MIN_PRICE = _get_env("GRIZZLY_MIN_PRICE","MIN_PRICE","minPrice")

PROVIDER_IDS = _get_env("GRIZZLY_PROVIDER_IDS","PROVIDER_IDS","providerIds")

EXCEPT_PROVIDER_IDS = _get_env("GRIZZLY_EXCEPT_PROVIDER_IDS","EXCEPT_PROVIDER_IDS","exceptProviderIds")

PHONE_EXCEPTION = _get_env("GRIZZLY_PHONE_EXCEPTION","PHONE_EXCEPTION","phoneException")

ARCHIVO = "creditos.json"

PAISES = {
    "pe": {"nombre": "🇵🇪 Perú", "codigo": 197}, "mx": {"nombre": "🇲🇽 México", "codigo": 187},
    "ar": {"nombre": "🇦🇷 Argentina", "codigo": 6}, "cl": {"nombre": "🇨🇱 Chile", "codigo": 176},
    "co": {"nombre": "🇨🇴 Colombia", "codigo": 181}, "es": {"nombre": "🇪🇸 España", "codigo": 146},
    "us": {"nombre": "🇺🇸 EEUU", "codigo": 12}, "ve": {"nombre": "🇻🇪 Venezuela", "codigo": 190},
    "br": {"nombre": "🇧🇷 Brasil", "codigo": 73}, "any": {"nombre": "🌍 Auto", "codigo": "any"},
}
...
PREMIUM={"1":"5431650332419563627","2":"6219810752887262728","3":"6298670698948724690","4":"5098585844931888090","5":"5260553279321944543","6":"5098578393163629920","7":"5429381339851796035","8":"5179570356695860413","9":"5177431372788139022","10":"5098536693326152842","11":"5260463209562776385","12":"5096114086958072826"}
...
async def procesar_spam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("esperando_spam_numero"):
        ...
        # No se ejecutan envíos masivos. Se conserva el flujo de entrada
        # para que el bot responda correctamente sin activar spam.
        await update.message.reply_text(
f"""╔══════════════════════════╗
...
🛡️ Los envíos masivos no se ejecutan desde este bot.
...
def keep_alive():
    class H(BaseHTTPRequestHandler):
        def do_GET(self): self.send_response(200); self.end_headers(); self.wfile.write(b"Bot V2 Final Alive")
        def log_message(self,*a): return
    port=int(os.getenv("PORT",10000))
    threading.Thread(target=lambda: HTTPServer(("0.0.0.0",port),H).serve_forever(), daemon=True).start()

def _patch(cls,mn):
    o=getattr(cls,mn,None)
    if not o or getattr(o,"_premium_global",False): return
    @functools.wraps(o)
    async def w(self,*a,**k):
        for ke in ("text","caption"):
            if ke in k and isinstance(k[ke],str): k[ke]=premium(k[ke])
        if "parse_mode" not in k: k["parse_mode"]="HTML"
        return await o(self,*a,**k)
    w._premium_global=True; setattr(cls,mn,w)

def instalar():
    for met in ("send_message","reply_text","edit_message_text","edit_text"):
        if hasattr(Message,met): _patch(Message,met)
        if hasattr(Bot,met): _patch(Bot,met)

def cargar():
    if not os.path.exists(ARCHIVO):
        with open(ARCHIVO,"w",encoding="utf-8") as f: json.dump({},f)
        return {}
    try:
...
def guardar(d):
    with open(ARCHIVO,"w",encoding="utf-8") as f: json.dump(d,f,indent=2)

def get_cred(uid): return cargar().get(str(uid),100)

def desc(uid,c):
    db=cargar(); a=db.get(str(uid),100)
    if a<c: return False
    db[str(uid)]=a-c; guardar(db); return True

def comprar_numero(service, country):
    params = {
    ...
    try:
        # Esta es la URL que pediste
        print(f"GET {BASE_URL} {params}")
        r = requests.get(BASE_URL, params=params, timeout=25)
        txt = r.text.strip()
        print(f"RESP: {txt[:1000]}")

        # Errores texto plano
        if txt.startswith("BAD_KEY") or txt=="NO_KEY": return {"ok":False,"err":"BAD_KEY - Clave API inválida"}
        if "NO_BALANCE" in txt: return {"ok":False,"err":"NO_BALANCE - Sin saldo ($3.70 te queda, pero Grizzly cobra en USD)"}
        if "NO_NUMBERS" in txt: return {"ok":False,"err":f"NO_NUMBERS - Sin stock para {country}/{service}. Prueba any"}
        if "BAD_ACTION" in txt: return {"ok":False,"err":f"BAD_ACTION - Acción no soportada. Respuesta: {txt}"}
        if "SERVICE_UNAVAILABLE_REGION" in txt: return {"ok":False,"err":"Región bloqueada, usa VPN"}

        # Éxito V2 es JSON como tu ejemplo
        try:
            j = r.json()
            # j = {"activationId":495357953,"phoneNumber":"18036181752",...}
            if "phoneNumber" in j and "activationId" in j:
                return {"ok":True,"data":j}
            else:
                return {"ok":False,"err":f"JSON inesperado: {j}"}
        except:
            return {"ok":False,"err":f"Respuesta no JSON: {txt[:500]}"}

    except Exception as e:
        return {"ok":False,"err":f"EXC: {e}"}

async def get_status(aid):
    try:
        r=requests.get(BASE_URL, params={"api_key":GRIZZLY_API_KEY,"action":"getStatus","id":aid}, timeout=15)
        t=r.text.strip()
        print(f"getStatus {aid} => {t}")
        if t.startswith("STATUS_OK:"): return t.split("STATUS_OK:")[1].strip()
        if t in ["STATUS_WAIT_CODE","STATUS_WAIT_RETRY","STATUS_WAIT_RESEND"]: return None
        if "STATUS_CANCEL" in t or "NO_ACTIVATION" in t: return "CANCELADO"
        return None
    except: return None

async def cancelar(aid):
    try:
        r=requests.get(BASE_URL, params={"api_key":GRIZZLY_API_KEY,"action":"setStatus","id":aid,"status":"8"}, timeout=15)
        print(f"cancel {aid} => {r.text}")
        return r.text.strip()
...
async def comprar(update, context):
    ...
    if len(context.args)<2:
        await update.message.reply_text("[1] Uso: <code>/comprar pe wa [cant]</code>\nEj: <code>/comprar pe wa</code> o <code>/comprar any wa</code>", parse_mode="HTML"); return
    kp=context.args[0].lower(); ks=context.args[1].lower()
    cant=1
    if len(context.args)>=3:
        try: cant=max(1,min(10,int(context.args[2])))
        except: cant=1
    if kp not in PAISES or ks not in SERVICIOS:
    ...
    oks=[]; errs=[]
    for _ in range(cant):
        res=comprar_numero(serv["codigo"], pais["codigo"])
        if res["ok"]:
            d=res["data"]
            # d tiene activationId, phoneNumber, activationCost, activationEnd, activationCancel
            oks.append(d)
            if uid not in numeros_activos: numeros_activos[uid]=[]
            numeros_activos[uid].append({
                "numero":d["phoneNumber"],
                "id":d["activationId"],
                "cost":d.get("activationCost"),
                "end":d.get("activationEnd"),
                "cancel":d.get("activationCancel")
            })
        else:
            errs.append(res["err"])
    ...
    if errs:
        tx+=f"[9] ❌ Errores:\n"+"\n".join(errs[:5])
        tx+=f"\n\n[3] Si es NO_NUMBERS prueba /comprar any wa"

    await msg.edit_text(tx, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb) if kb else None)

async def codigos(update, context):
    uid=str(update.effective_user.id)
    if uid not in numeros_activos or not numeros_activos[uid]:
        await update.message.reply_text("Sin números. /comprar", parse_mode="HTML"); return
    kb=[]; tx=f"[3] <b>TUS CÓDIGOS</b> {len(numeros_activos[uid])}\n\n"
    for it in numeros_activos[uid]:
        c=await get_status(it["id"])
        if c and c!="CANCELADO":
            tx+=f"📱 <code>+{it['numero']}</code> → <b>{c}</b> 🆔 {it['id']}\n\n"
            kb.append([InlineKeyboardButton(f"❌ Cancelar +{it['numero']}", callback_data=f"cancel_{it['id']}")])
        elif c=="CANCELADO": tx+=f"📱 +{it['numero']} ❌ Cancelado\n\n"
        else: tx+=f"📱 <code>+{it['numero']}</code> ⏳ Esperando... /verificar {it['id']}\n\n"; kb.append([InlineKeyboardButton(f"🔄 Ver {it['numero']}", callback_data=f"check_{it['id']}"), InlineKeyboardButton("❌ Cancelar", callback_data=f"cancel_{it['id']}")])
    await update.message.reply_text(tx, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb) if kb else None)

async def verificar(update, context):
    if not context.args: await update.message.reply_text("Uso: /verificar activationId", parse_mode="HTML"); return
    c=await get_status(context.args[0])
    await update.message.reply_text(f"✅ Código: <code>{c}</code>" if c and c!="CANCELADO" else "⏳ Aún no llega o cancelado", parse_mode="HTML")

async def cancelar_cmd(update, context):
    if not context.args: await update.message.reply_text("Uso: /cancelar activationId", parse_mode="HTML"); return
    r=await cancelar(context.args[0])
    uid=str(update.effective_user.id)
    if uid in numeros_activos: numeros_activos[uid]=[x for x in numeros_activos[uid] if str(x["id"])!=context.args[0]]
    await update.message.reply_text(f"❌ Cancelado: {r}", parse_mode="HTML")
...
async def botones(update, context):
    ...
    if d.startswith("cancel_"):
        ...
        if c and c!="CANCELADO": await q.message.reply_text(f"✅ Código {aid}: <code>{c}</code>", parse_mode="HTML")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(otros_callback, pattern=r"^(otros_|nu_comandos)$"))
    ...
    # ===================== NÚMEROS VIRTUALES / GRIZZLY =====================
    app.add_handler(CommandHandler("paises", paises_cmd))
