...
"""
        for rostro in rostros:
            mensaje += f"┌─ ROSTRO #{rostro.get('id', '-')}\n"
        await update.message.reply_text(mensaje + footer_creditos(context))
    except Exception as e:
        agregar_creditos(update.effective_user.id, COSTOS["facial"])
        await update.message.reply_text(f"❌ Error: {str(e)[:200]} — Créditos devueltos.")
    finally:
...
async def main():
    ...
    logger.info("🤖 Bot SPECTER PERÚ iniciado en modo POLLING")
